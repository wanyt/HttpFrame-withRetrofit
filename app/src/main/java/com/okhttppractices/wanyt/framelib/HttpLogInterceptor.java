...
public class HttpLogInterceptor implements Interceptor {
    ...
    @Override public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (!hasRequestBody) {
            logger.log("--> 请求结束 " + request.method());
        } else if (bodyEncoded(request.headers())) {
            logger.log("--> 请求结束 " + request.method() + " (encoded body omitted)");
        } else {
            ...
            if (isPlaintext(buffer)) {
                ...
                logger.log("--> 请求结束 " + request.method() + " (binary "
        ...
        logger.log("<-- 响应内容：" + response.code() + ' ' + response.message() + ' '
        ...
        if (bodyEncoded(response.headers())) {
            logger.log("<-- 响应结束 (encoded body omitted)");
        } else {
            ...
            if (!isPlaintext(buffer)) {
                logger.log("");
                logger.log("<-- 响应结束 (binary " + buffer.size() + "-byte body omitted)");
                return response;
            }
